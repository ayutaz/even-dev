...
実データに差し替えるには、`src/exhibitors-data.json` を更新し、`version` を 1 つ増やしてください。同じ形式の JSON を `main.ts` の `REMOTE_DATA_URL` に指定した公開 URL に置くと、通信できる環境で自動的に新しい方が使われます。その URL は省略可能で、取得に失敗してもアプリは完全にオフラインで動作します。
