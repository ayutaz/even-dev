...
**この計画は完了している。** 2026-08-03 に `main` へマージ済み（`36ff787..85d419f`、17 コミット）。テスト 82 件通過、`npx tsc --noEmit` クリーン、`out.ehpk` 生成確認済み。

各タスクは「実装 → レビュー → 必要なら修正 → 再レビュー」で進めた。修正ラウンドが発生したのは Task 2（`compareBooth`）、Task 7（同梱データの 404 と表示上限）、Task 8（ルート README への登録漏れ）、およびブランチ全体レビュー後の 3 件である。

## 実装中に確定した変更（この計画本文との差分）

以下は**このドキュメント本文のコード例より実際のコードが正しい**。各タスクのコードブロックは当時の指示内容として残してある。
...
4. **G2 コントローラの 3 点（Task 6、ブランチ全体レビュー後）** — 本文の `eventlens-app.ts` には次の不具合があった。
   - `onEvenHubEvent` の解除関数を捨てていたため、描画失敗後に再接続すると**リスナーが二重登録**され、タップ 1 回で訪問済みが 2 回反転して操作が効かなくなる。解除関数を保持し、再登録前と接続失敗時に呼ぶ形に修正した。
   - `getIncomingIndex` が端末の選択位置を採用していたため、カーソルの真実源が 2 つになっていた。SDK には選択位置を*書き込む*手段が無く、アプリ側が独自にカーソルを動かすとずれる。この関数を削除し、スクロールイベントによる相対移動だけに統一した。
   - コンテナの幅が `x=8, width=560` で描画領域を 8px 超えていた。幅を 544 にし、見出しの高さを 72 → 100 に広げた（配置は設計 10.4 の表を参照）。

5. **長い出展名の切り詰め（Task 6）** — 実測した行高（設計 13.1）では、高さ 100 の見出しに入るのは 3 行であり、ブース番号が 1 行を使うため名前に使えるのは 72 カラムである。同梱サンプルの最長は 80 カラムあり溢れる。`exhibitors.ts` に `displayColumns` / `truncateToColumns` を純粋関数として追加し（テスト 16 件）、見出しの名前を 72 カラム、リスト行を 36 カラムに収めるようにした。

6. **`app.json` の権限宣言（Task 8）** — 本文には記載が無かったが、`permissions` は `{name, desc}` の配列でなければ `pack` が通らない。`min_sdk_version` と `supported_languages` も必須である。通信先ホストを書くフィールドは存在しないため、`desc` に `www.genai-expo.com` と「ユーザーデータを送信しない」旨を明記した。詳細は設計 13.2 を参照。
...
コードは完成している。残っているのは主催側の作業であり、内容は[設計ドキュメントの 11 節](../specs/2026-08-02-eventlens-design.md)に集約した。要点は出展者データの差し替え（ブース番号は半角数字）、プライバシーポリシーの掲示先 URL、`out.ehpk` の審査提出（目標 2026-09-01）、実機での可読性確認である。
